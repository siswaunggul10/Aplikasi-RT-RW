...
\restrict iXFtDyKDuPjWeELcpSbkjgeYu55IfDS9wCeIBZk8ip8X0q54A4ZJkoFKSpeE9x8
...
DECLARE
  ...
  v_user_kk TEXT;
BEGIN
  -- 1. Validasi Token Sesi langsung ke tabel "Sessions"
  SELECT s.nik, LOWER(TRIM(s.role)) INTO v_user_nik, v_user_role
  FROM public."Sessions" s
  WHERE s.token = p_token
  LIMIT 1;

  -- Jika token tidak ada di database / ngasal
  IF v_user_nik IS NULL THEN
    RAISE EXCEPTION 'Akses ditolak! Sesi login tidak valid.';
  END IF;

  -- 2. Cari No KK milik user terverifikasi
  SELECT w.no_kk INTO v_user_kk 
  FROM public."Warga" w
  WHERE w.nik::text = v_user_nik::text LIMIT 1;

  -- 3. Jika Role terbukti 'rt' atau 'admin' di DB Sessions:
  ...
  -- 4. Jika Warga biasa, SENSOR data sensitif milik warga lain
  ELSE
    RETURN QUERY 
    SELECT 
      w.id,
      ...
      -- Sensor NIK
      CASE 
        WHEN (v_user_kk IS NOT NULL AND w.no_kk = v_user_kk) OR w.nik::text = v_user_nik::text 
        THEN w.nik::text 
        ELSE 'XXXXX' 
      END AS nik,
      -- Sensor No KK
...
DECLARE
    ...
    v_db_password TEXT;
BEGIN
    -- 0. Validasi Domain (Mengizinkan github.io & localhost)
    v_origin := COALESCE(current_setting('request.headers', true)::json->>'origin', '');
    IF v_origin <> '' AND v_origin NOT LIKE '%github.io%' AND v_origin NOT LIKE '%localhost%' THEN
        RETURN jsonb_build_object('status', 'error', 'message', 'Akses login ditolak: Domain tidak sah!');
    END IF;

    -- 1. Cari user di tabel Users (Cast nik::text agar aman dari error bigint)
    SELECT * INTO v_user
    FROM public."Users"
    ...
    -- 5. Jika Warga Biasa -> Cari profil di tabel Warga (Cast nik::text agar tidak error bigint = text)
    SELECT * INTO v_warga
    FROM public."Warga"
    WHERE COALESCE(nik::text, '') = v_nik::text
    LIMIT 1;
...
--
-- Name: Warga Allow Delete Warga RT Only; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Allow Delete Warga RT Only" ON public."Warga" FOR DELETE USING (true);


--
-- Name: Sessions Allow Select Sessions; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Allow Select Sessions" ON public."Sessions" FOR SELECT TO authenticated, anon USING (true);


--
-- Name: Users Allow Select Users; Type: POLICY; Schema: public; Owner: postgres
--
...
--
-- Name: Warga Allow Select Warga; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Allow Select Warga" ON public."Warga" FOR SELECT TO authenticated, anon USING (true);


--
-- Name: Warga Allow Update Warga RT Only; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Allow Update Warga RT Only" ON public."Warga" FOR UPDATE USING (true) WITH CHECK (true);
...
\unrestrict iXFtDyKDuPjWeELcpSbkjgeYu55IfDS9wCeIBZk8ip8X0q54A4ZJkoFKSpeE9x8
